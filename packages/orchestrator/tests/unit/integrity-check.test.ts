...
describe("Integrity Check", () => {
  ...
  it("produces a different checksum when the mission changes", () => {
    ...
    const altered = computeConfigChecksum("Do something else entirely.", baseConfig);
    expect(original).not.toBe(altered);
  });

  it("produces a different checksum when the config changes", () => {
    const original = computeConfigChecksum(baseMission, baseConfig);
    const altered = computeConfigChecksum(baseMission, { maxRetries: 5, timeout: 30_000 });
    expect(original).not.toBe(altered);
  });

  it("is not affected by config key ordering", () => {
    const a = computeConfigChecksum(baseMission, { timeout: 30_000, maxRetries: 3 });
    const b = computeConfigChecksum(baseMission, { maxRetries: 3, timeout: 30_000 });
    expect(a).toBe(b);
  });
