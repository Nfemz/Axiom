...
export default function AgentsPage() {
  ...
  useEffect(() => {
    if (!sseEvent || sseEvent.type === "keepalive") return;
  ...
  if (fetchError) return <div style={{ padding: "2rem", color: "#ef4444" }}>Error: {fetchError}</div>;
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      {agents.length === 0 ? (
        <p style={{ color: "#9ca3af" }}>No agents running. Spawn one from a definition.</p>
      ) : (
            ...
            {agents.map((agent) => (
              <tr key={agent.id} style={{ borderBottom: "1px solid #1f2937" }}>
                <td style={{ padding: "0.75rem" }}>
                  <a href={`/agents/${agent.id}`} style={{ color: "#60a5fa", textDecoration: "none" }}>
                    {agent.name}
                  </a>
                </td>
                <td style={{ padding: "0.75rem" }}>
                ...
                <td style={{ padding: "0.75rem", color: "#d1d5db", maxWidth: "300px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {agent.currentTask ?? "-"}
                </td>
                ...
                <td style={{ padding: "0.75rem", display: "flex", gap: "0.5rem" }}>
                  {agent.status === "running" && (
                    <button
                      ...
                      style={{ padding: "0.25rem 0.75rem", backgroundColor: "#eab308", color: "#000", border: "none", borderRadius: "4px", cursor: "pointer" }}
                    >
                      Pause
                    </button>
                  )}
                  {agent.status === "paused" && (
                    <button
                      ...
                      style={{ padding: "0.25rem 0.75rem", backgroundColor: "#22c55e", color: "#000", border: "none", borderRadius: "4px", cursor: "pointer" }}
                    >
                      Resume
                    </button>
                  )}
                  {agent.status !== "terminated" && (
                    <button
                      ...
                      style={{ padding: "0.25rem 0.75rem", backgroundColor: "#ef4444", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer" }}
                    >
                      Terminate
                    </button>
                  )}
